...
export function TermsPage(props: TermsPageProps) {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        const response = await fetch(`/content/terms/${i18n.language}.md`);
        if (response.ok) {
        ...
        // Fallback to English
        try {
          const response = await fetch('/content/terms/en.md');
          const text = await response.text();
          setContent(text);
        } catch (error) {
  ...
  const processedContent = content.replace('our support email', supportEmail);
